    ...
    (writer) => {
      ...
      for (const [path, methods] of Object.entries(spec.paths)) {
        ...
        for (const [_method, { operationId }] of Object.entries(methods)) {
          writer.writeLine(
            `${operationId}: (params: z.infer<typeof ${operationId}_Parameters>) => Promise<z.infer<typeof ${operationId}_ResponseBody>>;`
          );
        }
      ...
      for (const [path, methods] of Object.entries(spec.paths)) {
        for (const [method, { operationId }] of Object.entries(methods)) {
          ...
          writer.writeLine(`paramType: ${operationId}_Parameters,`);
          writer.writeLine(`responseType: ${operationId}_ResponseBody,`);
